...
function MsgRowOnMouseOverTB(msgRow, event) {
    if (!msgRow.active) {
        ...
        msgRow.effect = new Effect.Highlight(msgRow, {
            startcolor: '#000000',
            endcolor: '#333333',
            restorecolor: '#333333',
        ...
        if (downElement) {
            msgRow.tbeffect = Effect.Appear(downElement, {
                duration: EFFECT_DURATION
...
function MsgRowOnMouseOutTB(msgRow, event) {
    ...
    if (msgRow.active) {
        ...
        var downElement = $(msgRow).down('.toolbar_contents');
        if (downElement) {
            msgRow.tbeffect = Effect.Fade(downElement, {
                duration: EFFECT_DURATION
...
function XFade(content, last) {
    ...
    if (last) {
        ...
        last.effect = Effect.Fade(last, {
            duration: TAB_XFADE_DURATION
        });
    }
    ...
    content.effect = Effect.Appear(content, {
        duration: TAB_XFADE_DURATION
    });
}
...
function SubmitQuickMessageUpdate(rgFields) {
    ...
    if ($('activetab'))
        updateForm.appendChild(new Element('input', {
            type: 'hidden',
            name: 'activetab',
...
function HighlightInput(inputName) {
    ...
    if (tabeffect) {
        ...
        tabeffect.options.afterFinish = function() {
            Effect.ScrollTo(input, {
                offset: -200,
                duration: .5
            });
        };
...
function ReadAssociationValues(json) {
    ...
    hash.each(function(entry) {
        if ($('messageform')[entry.key]) $('messageform')[entry.key].value = entry.value;
    });
