...
function CommitAddPartnerBox() {
    ...
    new Ajax.Request(g_szBaseURL + '/admin/addpublisherajax', {
        ...
        onFailure: function(transport) {
            alert('Ajax call failed in CommitAddPartnerBox().');
        },
        onException: function(request, e) {
            alert('Exception during call to CommitAddPartnerBox().');
            throw e;
        }
    });
...
function OnChangeAllAppsAsync() {
    ...
    new Ajax.Request(g_szBaseURL + '/admin/allappsqueryajax', {
        ...
        onFailure: function(transport) {
            alert('Ajax call failed in OnChangeAllApps().');
        },
        onException: function(request, e) {
            alert('Exception during call to OnChangeAllApps().' + e);
            throw e;
        }
    });
}
...
function DisplayAllApps(response, resultsElement) {
    ...
    for (var i = 0; i < apps.length; ++i) {
        ...
        switch (parseInt(thisApp.app_type)) {
            ...
            default:
                color = '#ff0000';
                break; // If we don't recognize the type, let put a nice color :)
        }
...
function OnChangeAllPackagesAsync() {
    ...
    new Ajax.Request(g_szBaseURL + '/admin/allpackagesqueryajax', {
        ...
        onFailure: function(transport) {
            alert('Ajax call failed in OnChangeAllPackages().');
        },
        onException: function(request, e) {
            alert('Exception during call to OnChangeAllPackages().' + e);
            throw e;
        }
    });
}
...
function OnSavePackageAjax(bDupe, appitems) {
    ...
    new Ajax.Request(g_szBaseURL + '/admin/savepackageajax', {
        ...
        parameters: parameters,
        onSuccess: function(transport) {
            OnSaveCddbPackageCallback(transport);
        },
        onFailure: function(transport) {
            alert('Ajax call failed in OnSavePackageAjax().');
        },
        onException: function(request, e) {
            alert('Exception during call to OnSavePackageAjax().' + e);
            throw e;
        }
    });
}
...
function ActionOnPackages(parentElement, appIds, action) {
    ...
    new Ajax.Request(g_szBaseURL + '/admin/actiononpackagesajax', {
        ...
        onFailure: function(transport) {
            alert('Ajax call failed in ActionOnPackages().');
        },
        onException: function(request, e) {
            alert('Exception during call to ActionOnPackages().' + e);
            throw e;
        }
    });
}
...
function LocListenForEvents(id, lang) {
    ...
    if (!g_bLocSetToEnglishOnLoad) {
        ...
        setTimeout(function() {
            LocChangeLanguage(lang || 'english')
        }, 1);
    }
}
...
function ChangeLanguage(strTargetLanguage, bStayOnPage) {
    ...
    $J.post('https://partner.steamgames.com/actions/setlanguage/', {
...
function RetireAppInternal(nAppID, nPubID, bDMCARetire, strNotes, strRetireAction, bRemoveCommunityPresence, bPurgeContent, bVerboseOutput) {
    ...
    if (!strRetireAction.localeCompare('Banned', undefined, {
            sensitivity: 'accent'
        })) {
        strRetireError = 'Unable to ban appID';
        strRetireSuccess = 'Successfully banned appID ';
    } else if (!strRetireAction.localeCompare('DMCA', undefined, {
            sensitivity: 'accent'
        })) {
