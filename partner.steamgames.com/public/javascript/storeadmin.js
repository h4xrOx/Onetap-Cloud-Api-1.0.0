...
function OnFreeGameCheck(checkbox, appid) {
    if (checkbox.value) {
        ...
        new Effect.BlindDown('free_game_settings', {
            duration: 0.25
        });
    } else {
        $('alt_appid_input').value = '';
        new Effect.BlindUp('free_game_settings', {
            duration: 0.25
        });
    }
}
    ...
    new Ajax.Request('https://partner.steamgames.com/admin/store/gettaggedpackages/', {
        ...
        onSuccess: function(transport) {
            ...
            for (var i = 0; i < matchingItems.length; ++i) {
                ...
                if (option['packageid']) {
                    newElement = $J('<div/>', {
                        id: clusterName + '_clusterpackage_' + option['packageid'],
                        'class': option['cssClass'],
                        text: name
                    });
                } else if (option['itemid']) {
                    newElement = $J('<div/>', {
                        id: clusterName + '_clusteritem_' + option['itemid'],
                        'class': option['cssClass'],
                        text: name
                    });
                }
        ...
        onSuccess: function(transport) {
            ...
            for (var i = 0; i < matchingItems.length; ++i) {
                ...
                if (option['packageid']) {
                    ...
                    newElement = $J('<div/>', {
                        id: clusterName + '_clusterbundle_' + option['bundleid'],
                        'class': option['cssClass'],
                        text: name
                    });
                }
    ...
    for (var i = 0; i < rgIncludedItems.length; i++) {
        ...
        if (rgItem.itemid) {
            var opt = new Element('div', {
                id: clusterName + '_clusteritem_' + rgItem.itemid,
                'class': g_rgReferencedItems[rgItem.itemid]['cssClass']
            });
            opt.innerHTML = g_rgReferencedItems[rgItem.itemid]['name'];
            elemIncludedApps.appendChild(opt);
        } else if (rgItem.packageid) {
            var opt = new Element('div', {
                id: clusterName + '_clusterpackage_' + rgItem.packageid,
                'class': 'app_Package'
            });
            ...
            var opt = new Element('div', {
                id: clusterName + '_clusterbundle_' + rgItem.bundleid,
                'class': 'app_Package'
            });
...
function CreateClusterSortable(elem) {
    ...
    Sortable.create(elem, {
        tag: 'div',
        scroll: $(elem).up('.appselect_list_ctn')
    });
}
...
function PreviewCapsules(strSize, elemIncluded) {
    ...
    url += Object.toQueryString({
        strCapsuleJSON: Object.toJSON(rgItems),
        strCapsuleSize: strSize
    });
...
function ImageHoverPreview(event, divHover, url) {
    ...
    if (!hover.visible() || hover.hiding) {
        ...
        hover.effect = Effect.Appear(hover, {
            duration: 0.2
        });
    }
...
function HideImageHover(event, divHover, elem) {
    ...
    if (hover.effect) hover.effect.cancel();
    if (hover.visible()) {
        hover.effect = Effect.Fade(hover, {
            duration: 0.2
        });
        hover.hiding = true;
    }
...
function OnGenreSelect(checkbox, id, name) {
    ...
    if (checkbox.value) {
        var elemOpt = new Element('option', {
            value: id
        });
        elemOpt.update(name);
