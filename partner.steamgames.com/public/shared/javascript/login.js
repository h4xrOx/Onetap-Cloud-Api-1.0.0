...
function CLoginPromptManager(strBaseURL, rgOptions) {
    ...
    $J('#login_btn_signin').children('a, button').click(function() {
        _this.DoLogin();
    });
    ...
    this.m_$ModalAuthCode.find('[data-modalstate]').each(function() {
        $J(this).click(function() {
            _this.SetEmailAuthModalState($J(this).data('modalstate'));
        });
    });
    ...
    this.m_$ModalTwoFactor.find('[data-modalstate]').each(function() {
        $J(this).click(function() {
            _this.SetTwoFactorAuthModalState($J(this).data('modalstate'));
        });
    });
    ...
    if (typeof BigNumber == 'undefined')
        $J.ajax({
            url: 'https://partner.steamgames.com/public/shared/javascript/crypto/jsbn.js',
            type: 'get',
            dataType: 'script',
            cache: true
        });
    if (typeof RSA == 'undefined')
        $J.ajax({
            url: 'https://partner.steamgames.com/public/shared/javascript/crypto/rsa.js',
            type: 'get',
            dataType: 'script',
            cache: true
        });
...
CLoginPromptManager.prototype.BIsIos = function() {
    return this.m_strMobileClientType == 'ios';
};
CLoginPromptManager.prototype.BIsAndroid = function() {
    return this.m_strMobileClientType == 'android';
};
CLoginPromptManager.prototype.BIsWinRT = function() {
    return this.m_strMobileClientType == 'winrt';
};
...
CLoginPromptManager.prototype.DoLogin = function() {
    ...
    var _this = this;
    $J.post(this.m_strBaseURL + 'getrsakey/', this.GetParameters({
            username: username
        }))
    ...
    if (results.login_complete) {
        ...
        if ((results.transfer_url || results.transfer_urls) && results.transfer_parameters) {
            ...
            if (!this.m_bLoginTransferInProgress)
                CLoginPromptManager.TransferLogin(results.transfer_urls || [results.transfer_url], results.transfer_parameters, function() {
...
CLoginPromptManager.prototype.LoginComplete = function() {
    if (this.m_fnOnSuccess) {
        ...
        if (this.m_strRedirectURL.match(/^\/\//)) {
            this.m_strRedirectURL = '';
        }
...
CLoginPromptManager.prototype.SetTwoFactorAuthModalState = function(step) {
    ...
    if (step == 'entercode') {
        ...
        if ($J('#twofactorcode_entry').val().length == 0) {
            ...
            var rgParameters = {
                rcode: $J('#twofactorcode_entry').val()
            };
