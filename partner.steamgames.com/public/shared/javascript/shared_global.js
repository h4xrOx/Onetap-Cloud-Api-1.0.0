...
CModal.prototype.Show = function() {
    ...
    if (typeof GPNavFocusChild !== 'undefined')
        window.setTimeout(function() {
            GPNavFocusChild(_this.m_$Content)
        }, 1);
};
                ...
                if (data && data.transfer_urls && data.transfer_params) {
                    for (var i = 0; i < data.transfer_urls.length; i++) {
                        $J.ajax({
                            ...
                            data: {
                                transfer_params: data.transfer_params
                            },
                            crossDomain: true,
                            xhrFields: {
                                withCredentials: true
...
function CAjaxInfiniteScrollingControls(rgSearchData, url) {
    ...
    this.m_fnRawScrollHandler = function() {
        thisControl.OnScroll()
    };
...
function BindTooltips(selector, rgOptions) {
    ...
    if ($TextTooltips.length)
        $TextTooltips.v_tooltip({
            ...
            'responsiveMode': window.UseSmallScreenMode && window.UseSmallScreenMode()
        });
...
window.addEventListener('vgp_gamepadnavready', function() {
    g_rgQueuedGamepadCommands.forEach(function(command) {
        window[command.name].apply(window, command.args);
    });
    g_rgQueuedGamepadCommands = [];

    g_rgOnReadyCallbacks.forEach(function(fn) {
        fn();
    });
...
function InstrumentFocusElements(element) {
    g_rgQueuedGamepadCommands.push({
        name: 'InstrumentFocusElements',
        args: arguments
    });
}
...
function GPNavUpdateActionDescriptions(element, actionDescriptions) {
    g_rgQueuedGamepadCommands.push({
        name: 'GPNavUpdateActionDescriptions',
        args: arguments
    });
}