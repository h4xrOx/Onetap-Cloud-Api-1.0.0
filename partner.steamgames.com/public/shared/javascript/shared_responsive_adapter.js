...
jQuery(function($) {
    ...
    var mqMobileMode = window.matchMedia ? window.matchMedia("(max-width: 500px)") : {
        matches: false
    };
    ...
    if ($LocalMenuContent.length) {
        ...
        GPNavUpdateActionDescriptions($('#responsive_page_template_content'), {
            onOptionsActionDescription: 'Menu'
        });
        $('#responsive_page_template_content').on('vgp_onoptions', function() {
            LocalMenuEvents.fnActivateMenu();
        });
    }
...
function Responsive_BuildChangeLanguageOption($MenuItem) {
    $MenuItem.click(function() {
        ...
        $SelectBox.append(
            $J('<option/>', {
                value: "latam"
            }).text('Español - Latinoamérica (Spanish - Latin America)')
        );
        $SelectBox.append(
            $J('<option/>', {
                value: "greek"
...
function Responsive_ReparentItemsInMobileMode(strItemSelector, $CtnOrFn) {
    return _Responsive_ReparentItems(strItemSelector, $CtnOrFn, function() {
        return window.UseMobileScreenMode && window.UseMobileScreenMode();
...
function Responsive_ReparentItemsInTabletMode(strItemSelector, $CtnOrFn) {
    return _Responsive_ReparentItems(strItemSelector, $CtnOrFn, function() {
        return window.UseTabletScreenMode && window.UseTabletScreenMode();
    }, 'Responsive_TabletScreenModeToggled');
}
...
function Responsive_ReparentItemsInResponsiveMode(strItemSelector, $CtnOrFn) {
    ...
    }, 'Responsive_SmallScreenModeToggled');
}
